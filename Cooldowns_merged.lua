...
-- Generated on: 2025-10-15T16:31:51.291Z
...
local cooldowns_player, cooldowns_enemy
...
function ADDON_LOADED()
    ...
    cooldowns_player = cooldowns_require 'cooldowns.player'
    cooldowns_enemy = cooldowns_require 'cooldowns.enemy'
...
local cooldowns_frame = cooldowns_require 'cooldowns.frame'
...
function cooldowns_module(name) if not loaded[name] then create_module(name) end setfenv(2, environments[name]) end
function cooldowns_library(name) if loaded[name] then _G.error(nil) end create_module(name) setfenv(2, environments[name]) end
function cooldowns_require(name) if not loaded[name] then create_module(name) end return interfaces[name] end
